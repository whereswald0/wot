...
/**
 * Calculates dew point in Celcius
 */
float SHT1x::calcDewPointC()
{
    float _temperature;         // Temperature value (Celcius)
    float _H;                   // place holder for Magnus dew point calculation
    float _Td;                  // dew point
    float _humidity;            // value for humidity

    _humidity = readHumidity();
    _temperature = readTemperatureC();

    _H = (log10(_humidity)-2)/(0.4343)+(17.62*_temperature)/(243.12+_temperature); // 17.62 is the Beta constant and 243.12 is lambda in Centigrade
    _Td = (243.12*_H)/(17.62-_H); //Dew point in Celcius calculated from Magnus equation
    //_Dpf = (Dpc *1.8)+32; // Dew point in Fahrenheit
    return(_Td);
}
 ...
 */
float SHT1x::calcDewPointF()
{
    float _TdC;                 // Dew point in Celcius
    float _TdF;                 // Dew point in Fahrenheit
